...
// مفاتيح كاش اللوحة — موحَّدة هنا حتى تُبطَل من أي مكان في المشروع
// بنفس الاسم بالضبط (يستخدمها أيضًا CaseManagement.gs و CheckEngine.gs)
const مفتاح_كاش_النظرة_العامة = 'نظرة_عامة_المشاريع';
const مفتاح_كاش_الحالات_المعلقة = 'حالات_بانتظار_القرار';

/**
 * يبطل كاش اللوحة بالكامل (النظرة العامة + الحالات المعلقة) — يُستدعى
 * من أي مكان في المشروع يكتب على جدول "الرئيسية" أو أي جدول تفاصيل
 * مشروع، حتى لا تعرض اللوحة بيانات قديمة قبل انتهاء صلاحية الكاش.
 */
function إبطال_كاش_اللوحة_() {
  const كاش = CacheService.getScriptCache();
  كاش.remove(مفتاح_كاش_النظرة_العامة);
  كاش.remove(مفتاح_كاش_الحالات_المعلقة);
}
...
function جلب_النظرة_العامة_() {
  const مفتاح_الكاش = مفتاح_كاش_النظرة_العامة;
...
function جلب_الحالات_بانتظار_القرار_() {
  const مفتاح_الكاش = مفتاح_كاش_الحالات_المعلقة;
  const مخزَّن = CacheService.getScriptCache().get(مفتاح_الكاش);
