...
const نافذة_النسخ = document.getElementById('نافذة-النسخ');
const حالة_النسخ = document.getElementById('حالة-النسخ');
const نص_النسخ = document.getElementById('نص-النسخ');
const زر_نسخ_يدوي = document.getElementById('زر-نسخ-يدوي');
const زر_إغلاق_النافذة = document.getElementById('زر-إغلاق-النافذة');
...
function رسم_قسم_الحالات_(حالات) {
  ...
  قسم_الحالات.querySelectorAll('[data-فهرس]').forEach(function (زر) {
    زر.addEventListener('click', function () {
      نسخ_تفاصيل_الحالة_(حالات[Number(زر.dataset.فهرس)]);
    });
  });
}

/**
 * يبني نصًا كاملًا (المشروع، النوع، الشدة، الوصف، الخطة المقترحة)
 * ويعرضه دائمًا في نافذة واضحة — بغض النظر عن نجاح النسخ التلقائي
 * للحافظة أو فشله، حتى لا يُفقَد أي تفصيل أبدًا. النص مكتفٍ ذاتيًا
 * بالكامل (لا يفترض أي سياق سابق)، ليصلح للصقه في أي محادثة جديدة
 * مع Claude Code لاحقًا حتى لو لم تكن لديه ذاكرة عن هذه الحالة.
 */
async function نسخ_تفاصيل_الحالة_(حالة) {
  const نص =
    ...
    'الوصف (المشكلة): ' + حالة['الوصف'] + '\n' +
    'الخطة المقترحة (الحل): ' + حالة['الخطة المقترحة'];

  نص_النسخ.value = نص;
  نافذة_النسخ.classList.remove('hidden');

  try {
    await navigator.clipboard.writeText(نص);
    حالة_النسخ.textContent = '✓ تم نسخ النص كاملًا لحافظتك — الصقه في محادثة جديدة مع Claude Code.';
  } catch (خطأ) {
    حالة_النسخ.textContent = 'تعذّر النسخ التلقائي — اضغط "نسخ النص" أدناه، أو انسخه يدويًا من الصندوق.';
    نص_النسخ.select();
  }
}

زر_نسخ_يدوي.addEventListener('click', async function () {
  try {
    نص_النسخ.select();
    await navigator.clipboard.writeText(نص_النسخ.value);
    حالة_النسخ.textContent = '✓ تم النسخ لحافظتك.';
  } catch (خطأ) {
    نص_النسخ.select();
    document.execCommand('copy');
    حالة_النسخ.textContent = '✓ تم تحديد النص — إن لم يُنسخ تلقائيًا، اضغط Ctrl+C.';
  }
});

زر_إغلاق_النافذة.addEventListener('click', function () {
  نافذة_النسخ.classList.add('hidden');
});
